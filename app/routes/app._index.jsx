 ...
 * - Sin loader ni @shopify/app-bridge (evita errores de Vercel).
 * - Editor abre con ?host= (base64url) + fallbacks a ?shop= y /admin/...
 * - Idiomas ES/EN/PT SIN auto-referencias (evita crashes SSR).
 */
...
const EN = {
  ...
  `,
  // Reuse the same detailed HTML content from ES to ensure completeness
  productsHtml: ES.productsHtml,
  collectionsHtml: ES.collectionsHtml,
  pagesHtml: ES.pagesHtml,
  blogHtml: ES.blogHtml,
  globalHtml: ES.globalHtml,
  suppressorHtml: ES.suppressorHtml,
};
...
const PT = {
  ...
  `,
  // Reuse ES detailed content to keep parity
  productsHtml: ES.productsHtml,
  collectionsHtml: ES.collectionsHtml,
  pagesHtml: ES.pagesHtml,
  blogHtml: ES.blogHtml,
...
/* ------------------------ STRINGS (sin auto-referencias) ------------------------ */
const STRINGS = { es: ES, en: EN, pt: PT };
...
export default function Dashboard() {
  ...
  // Botón editor: decodifica ?host= (base64url) → admin path; fallbacks a ?shop= y /admin/...
  const openThemeEditor = () => {
    try {
      const params = new URLSearchParams(window.location.search);
      const hostParam = params.get("host");
      if (hostParam) {
        // base64url → base64 (reemplaza - _ y rellena =)
        ...
        // ejemplos decode: "shop.myshopify.com/admin" o "admin.shopify.com/store/xxxx"
        if (decoded) {
      ...
      alert("No pude abrir el editor. Abre la app desde el Admin (URL con ?host=...) o añade ?shop=xxxx.myshopify.com.");
    }
  };
